...
export function createConfigs(sources: SourceContext): Record<string, ScreenConfig> {
  ...
  return {
    'strategy-map': {
      ...
      dependency: 'Теория проекта → Диагностика → Стратегический выбор → Целевое состояние → Стратегическая карта',
      ...
      sourceCards: s => [
        ...
        { title: 'Стратегический выбор', value: s.strategy.guidingPolicy || s.strategy.acceptedChoice || 'Сначала заполните Стратегический выбор', note: 'Направляющая политика задаёт логику карты.' },
        { title: 'Диагностика', value: s.diagnosis.keyChallenge || s.diagnosis.finalStatement || 'Сначала заполните Диагностику', note: 'Карта должна отвечать на главное препятствие.' },
      ],
      prefill: s => ({
    ...
    hypotheses: {
      ...
      dependency: 'Диагностика / Стратегический выбор / Целевое состояние / Стратегическая карта → Гипотезы',
      cardName: 'Гипотеза',
      ...
      sourceCards: s => [
        ...
        { title: 'Диагностика', value: s.diagnosis.keyChallenge || s.diagnosis.finalStatement || 'Сначала заполните Диагностику', note: 'Гипотеза должна быть связана с препятствием.' },
      ],
    ...
    decisions: {
      ...
      dependency: 'Проверки / Факты / Гипотезы / Диагностика / Стратегический выбор → Решения',
      cardName: 'Решение',
      ...
      sourceCards: s => [
        { title: 'Направляющая политика', value: s.strategy.guidingPolicy || 'Сначала заполните Стратегический выбор', note: 'Решение должно быть совместимо с направляющей политикой.' },
      ...
      fields: [
        ...
        { key: 'excluded', label: 'Что исключаем / компромисс', type: 'textarea' },
        { key: 'guidingPolicy', label: 'Связь с направляющей политикой', type: 'textarea' },
    ...
    initiatives: {
      id: 'initiatives',
      title: 'Инициативы',
      lead: 'Инициатива собирает комплекс действий, который реализует выбор, карту, цели, способности и системы управления.',
      ...
      sourceCards: s => [
        { title: 'Стратегический выбор', value: s.strategy.acceptedChoice || s.strategy.guidingPolicy || 'Сначала заполните Стратегический выбор', note: 'Инициатива должна поддерживать «где конкурируем» / «как выигрываем» / способности / системы управления.' },
    ...
    'business-processes': {
      ...
      fields: [
        ...
        { key: 'classification', label: 'Классификация по стратегической карте', type: 'select', options: ['управление операциями', 'управление клиентами', 'инновации', 'регуляторные и социальные процессы'] },
    ...
    tasks: {
      id: 'tasks',
      title: 'Задачи',
      lead: 'Задача превращает инициативы, процессы, проверки и решения в закреплённую ответственность: результат, срок, ответственность и подтверждение.',
      ...
      sourceCards: s => [
        { title: 'Согласованные действия', value: s.strategy.actions.map(item => item.label).join(', ') || 'Сначала заполните действия в Стратегическом выборе', note: 'Задачи должны быть связаны с выбранными действиями.' },
      ...
      fields: [
        { key: 'source', label: 'Источник задачи', type: 'select', options: ['инициатива', 'согласованное действие', 'решение', 'бизнес-процесс', 'OKR', 'проверка'] },
        ...
        { key: 'feedback', label: 'Обратная связь по результатам', type: 'textarea' },
        { key: 'status', label: 'Статус', type: 'select', options: ['бэклог', 'в работе', 'заблокирована', 'выполнена', 'отменена', 'требует пересмотра'] },
      ],
    },
    'facts-learning': {
      id: 'facts-learning',
