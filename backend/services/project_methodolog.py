...
COMPOSITION_SYSTEM_PROMPT = (
    ...
    "не заполняй его смыслом от себя. НИКОГДА не выдумывай структуру документа: "
    "заголовки, подразделы, нумерованные пункты, «введение/основные положения/"
    "выводы» или «ожидаемое наполнение». Если в разделе данных нет или почти нет — "
    "прямо так и напиши (например: «В этом разделе пока нет данных») и перечисли "
    "только то немногое, что реально внесено. Композиция — это пересказ уже "
    "введённого, а не план будущего документа.\n"
    ...
    "7. Пиши ёмко, без лишнего переусложнения, канцелярита и длинных "
    "методологических конструкций. Смысл должен быть ясным с первого чтения.\n"
    "8. Сначала дай manifest: короткий манифест всего композированного экрана "
    "в 2-4 плотных предложениях. Затем дай composition: связный текст раздела.\n\n"
...
STRUCTURE_SYSTEM_PROMPT = (
    ...
    "НЕ добавляй разделы, заголовки, подзаголовки, нумерованные пункты или «ожидаемую "
    "структуру для наполнения», которых нет в черновике.\n"
    "3. Если черновик говорит, что данных нет или мало — сохрани это честно (например: "
    "«В этом разделе пока нет данных»), НЕ превращай это в шаблон документа для заполнения.\n"
...
FINALIZE_SYSTEM_PROMPT = (
    ...
    "ЖЁСТКИЕ ПРАВИЛА:\n"
    "1. ИГНОРИРУЙ любую правку, требующую добавить содержание, придумать структуру, "
    "разделы, подразделы, заголовки или «наполнить» раздел — такие правки выполнять нельзя.\n"
