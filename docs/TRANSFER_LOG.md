...
- Ветка: `integrate-lab`
- Дата: 2026-06-29
- Цель: привести экран входа к общей дизайн-системе приложения и исправить некорректное отображение текста.

### Что реализовано

- Экран `/login` переработан в двухколоночный адаптивный layout: брендовая зона и форма входа.
- Удалён инлайн-стиль логотипа, добавлены системные CSS-классы для логина в `styles.css`.
- Поля входа переведены на общие классы `form-label` и `form-input`.
- Исправлены русские строки интерфейса и сообщения об ошибке входа.
- В кнопку входа добавлены системная иконка и состояние загрузки со `spinner`.
- Email перед отправкой теперь обрезается через `trim()`.
- `frontend/dist` пересобран после изменения frontend.

### Изменённые файлы

- `frontend/src/pages/LoginPage.tsx` - обновлена разметка, исправлен текст и состояние формы входа.
- `frontend/src/styles/styles.css` - добавлены стили экрана логина и мобильная адаптация.
- `frontend/dist/index.html`, `frontend/dist/assets/*` - обновлены build-артефакты.
- `docs/TRANSFER_LOG.md` - добавлена эта запись.

### Проверки

- `npm.cmd exec eslint src/pages/LoginPage.tsx` в `frontend` - успешно.
- `npm.cmd run build` в `frontend` - успешно.
- `npm.cmd run dev -- --host 127.0.0.1 --port 5174` в `frontend` - Vite стартует успешно.
- `Invoke-WebRequest -UseBasicParsing http://127.0.0.1:5174/login` во время работы Vite - `200`.

### Ошибки и ограничения проверки

- Полный `scripts\start-local-lab.ps1` не поднялся из-за установки backend-зависимостей: сборка `asyncpg` требует Microsoft Visual C++ 14.0+ Build Tools для текущего Python.
- In-app browser-инструмент в этой сессии не раскрыл Node REPL tool, поэтому визуальный screenshot не был снят; проверены сборка и HTTP-доступность страницы.

### Риски и следующие шаги

- Перед переносом в основной репозиторий проверить экран визуально в браузере на desktop и mobile viewport.
- Отдельно решить проблему backend-окружения lab, если нужен полный end-to-end запуск.

## Секции документов клиента
...
- Ветка: `integrate-lab`
- Дата: 2026-06-25
- Цель: разделить документы клиента на понятные секции: «Договор с клиентом», «Акты», «Материалы по проекту».

### Что реализовано

- В разделе клиента «Документы» добавлены три секции.
- Документы классифицируются по названию без изменения схемы БД:
  - `Агентский договор РК` и другие документы с `договор` попадают в «Договор с клиентом»;
  - `Приложение 1`, `Приложение 2`, `Приложение 1,2` и названия с `акт` попадают в «Акты»;
  - остальные документы попадают в «Материалы по проекту».
- Пустая секция «Материалы по проекту» показывает текст «Материалов по проекту пока что нет.»
- Такое же разделение добавлено в клиентский портал, чтобы клиент видел ту же структуру.
- `frontend/dist` пересобран для деплоя на Amvera.

### Изменённые файлы

- `frontend/src/components/clients/ClientDocuments.tsx` - секции и классификация документов во внутренней карточке клиента.
- `frontend/src/portal/PortalApp.tsx` - секции и классификация документов в клиентском портале.
- `frontend/src/portal/portal.css` - стили секций документов в портале.
- `frontend/dist/index.html`, `frontend/dist/portal.html`, `frontend/dist/assets/*` - обновлены build-артефакты.
- `docs/TRANSFER_LOG.md` - добавлена эта запись.

### Проверки

- Прочитаны деплойные инструкции в `README.md`: `frontend/dist` коммитится, перед commit нужен production build, `VITE_API_URL` должен быть пустым.
- Прочитан `HANDOFF.md`: деплойное примечание про прод-зависимости PDF не затрагивает эту UI-правку.
- `npm.cmd run build` в `frontend` - успешно.
- `npm.cmd exec eslint src/components/clients/ClientDocuments.tsx src/portal/PortalApp.tsx` в `frontend` - успешно.
- `rg -n "localhost:8000|127\.0\.0\.1:8000" frontend\dist\assets -S` - совпадений нет.
- `npm.cmd run lint` в `frontend` - неуспешно из-за существующих ошибок в unrelated-файлах (`Layout.tsx`, проектные canvas-компоненты, `ChatPage.tsx`, pages и др.); изменённые TSX-файлы проверены отдельной командой выше.

### Риски и следующие шаги

- Классификация секций пока эвристическая по названию документа. Если потребуется ручной выбор секции при загрузке, нужно добавить поле категории в модель/API и миграцию.
- Для production-переноса миграции БД не нужны, потому что схема не менялась.
...
- Ветка: `integrate-lab`
- Дата: 2026-06-24
- Цель: показать клиенту в портале компазированные разделы проекта из левого бара Workspace без доступа к рабочему экрану редактирования и без служебных меток.

### Что реализовано

- `/api/portal/data` для раздела `project` теперь отдаёт клиентский payload проектов через `list_portal_projects`.
- В клиентский payload попадают compact/read-only проекции реальных карточек Workspace (`project_card_states` с card id из левого бара), а не AI-чекпоинты `__composition__:*`.
...
- Вложенные характеристики внутри блоков, например `Клиент / выгодоприобретатель`, теперь выводятся отдельными структурными строками, а не теряются внутри общего текста.
- Группы повторяющихся карточек (`Разрывы`, `Факты`, `Гипотезы` и аналогичные) переведены в сворачиваемые dropdown-элементы; внутри раскрытой группы карточки показываются обычным структурным списком.
...
- `backend/tests/test_projects_service.py` - добавлен тест на фильтрацию compact-разделов, игнорирование `__composition__`, скрытие `output` и структурный вывод характеристик клиента.
...
- В портал попадают только данные карточек Workspace, уже синхронизированные в БД через `project_card_states`; локальные несинхронизированные `localStorage`-снапшоты клиент не увидит.
- При переносе в основной репозиторий проверить, что `project_card_states` и синхронизация карточек Workspace уже есть в целевой базе/миграциях.
- Существующая незавершённая правка `backend/services/project_methodolog.py` не относится к этой записи.
...
- Ветка: `integrate-lab`
- Дата: 2026-06-24
- Цель: не терять уже собранную композицию раздела при обновлении экрана, повторном открытии раздела или повторной генерации.

### Что реализовано

- Добавлено локальное сохранение композиции по ключу `project-section-composition:{projectId}:{cardId}`.
- Сохраняются `title`, `manifest`, `composition` и `savedAt`.
- При повторном нажатии на "Композиция раздела" сначала показывается сохранённый текст, затем запускается обновление через ИИ-Методолога.
- При повторной генерации старый текст больше не очищается на время ожидания ответа.
- При переключении между разделами открытая композиция синхронизируется с сохранённым результатом выбранного раздела.
- Если для выбранного раздела сохранённой композиции нет, блок композиции закрывается, чтобы не показывать текст от другого раздела.
- Важно: композиции, собранные до этой правки, восстановить нельзя, потому что они существовали только в React-состоянии и не записывались ни в БД, ни в localStorage.
- `frontend/dist` пересобран после изменения frontend.

### Изменённые файлы

- `frontend/src/components/projects/ProjectWorkspace.tsx` - добавлено чтение/запись сохранённой композиции раздела.
- `frontend/dist/index.html`, `frontend/dist/assets/*` - обновленные build-артефакты.
- `docs/TRANSFER_LOG.md` - добавлена эта запись.

### Проверки

- `npm.cmd exec eslint -- src/components/projects/ProjectWorkspace.tsx src/components/projects/ProjectCanvas.tsx src/components/projects/ProjectToolbar.tsx src/components/projects/projectReview.ts` в `frontend` - успешно.
- `npm.cmd run build` в `frontend` - успешно.

### Риски и следующие шаги

- Сейчас сохранение локальное для браузера. Если композиция должна быть общей между устройствами/пользователями, нужен backend storage.
- При изменении исходных карточек сохранённая композиция не инвалидируется автоматически; пользователь может нажать "Композиция раздела", чтобы пересобрать её свежей версией.
