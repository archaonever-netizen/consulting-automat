...
## Композиции проекта в клиентском портале
...
- Ветка: `integrate-lab`
- Дата: 2026-06-24
- Цель: показать клиенту в портале готовые композиции разделов проекта без доступа к рабочему экрану редактирования и без служебных меток.

### Что реализовано

- `/api/portal/data` для раздела `project` теперь отдаёт клиентский payload проектов через `list_portal_projects`.
- В клиентский payload попадают только завершённые композиции разделов (`status: done`, `final`).
- Внутренние данные не отдаются: editable card content, validation, чат методолога, черновики, рекомендации рецензента и стадии сборки.
- В портале раздел `Проект` показывает проекты и свёрнутые клиентские разделы с готовыми композициями.
- Внутри разделов отображаются только короткая суть и связный текст, без рабочих названий вроде "Манифест", прогресса или технических статусов.
...
- `backend/services/projects.py` - добавлена сборка публичных проектных разделов для портала.
- `backend/tests/test_projects_service.py` - добавлен тест на фильтрацию завершённых публичных композиций.
...
- В портал попадают только композиции, уже сохранённые в БД через многоэтапную сборку; старые локальные `localStorage`-композиции клиент не увидит.
- При переносе в основной репозиторий проверить, что `project_card_states` и сохранение композиции уже есть в целевой базе/миграциях.
